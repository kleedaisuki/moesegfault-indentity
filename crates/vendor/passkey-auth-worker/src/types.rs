...
///
/// On native targets this uses [`std::time::SystemTime`]. On WebAssembly it
/// deliberately uses the host JavaScript clock because `SystemTime::now()`
/// panics on `wasm32-unknown-unknown` in Workers.
///
/// 当前 Unix 秒时间戳，用于标记仪式状态。原生目标使用
/// [`std::time::SystemTime`]；WebAssembly 目标使用 JavaScript 宿主时钟，避免
/// Workers 的 `wasm32-unknown-unknown` 运行时调用 `SystemTime::now()` 时 panic。
...
/// Current Unix time in seconds from the JavaScript host clock.
///
/// Non-finite and pre-epoch values map to zero, matching the native fallback
/// without changing the ceremony expiry rules.
///
/// 从 JavaScript 宿主时钟读取当前 Unix 秒时间戳。非有限值与纪元前时间映射为
/// 零，与原生回退行为一致，且不改变仪式过期规则。
#[cfg(all(target_arch = "wasm32", target_os = "unknown"))]
pub(crate) fn now_secs() -> u64 {
    let millis = js_sys::Date::now();
    if millis.is_finite() && millis >= 0.0 {
        (millis / 1_000.0).floor() as u64
    } else {
        0
    }
}
...
mod clock_tests {
    ...
    /// Ensures the native abstraction stays aligned with Unix wall-clock time.
    /// 确保原生时钟抽象与 Unix 墙上时钟保持一致。
    #[test]
    fn now_secs_is_between_native_clock_samples() {
